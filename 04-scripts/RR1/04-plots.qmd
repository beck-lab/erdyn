...
summ_pwr <- nested_dat %>%
  ...
  full_join(summ_df) %>%
  filter(
    emo_item %in% c("Positive", "Negative") & 
    ((parameter == "beta_E" & model == "RQ1") | 
    (parameter == "beta_ExER" & model == "RQ2-int"))
    ) %>%
  mutate(sig = ifelse(sign(lower) == sign(upper), "sig", "ns"))

summ_pwr %>%
  group_by(emo_item, model, ergoal, er_exc, parameter, exclusion, sig) %>%
  tally() %>%
  ggplot(aes(x = sig, y = n)) + 
    geom_col() + 
    facet_wrap(~model + emo_item + ergoal + er_exc + exclusion) + 
    my_theme()
...
summ_pwr %>%
  ...
  ggplot(aes(x = n)) + 
    ...
    facet_wrap(~emo_item + ergoal + er_exc + exclusion) + 
    my_theme()

summ_pwr %>%
  group_by(model, emo_item, ergoal, er_exc, exclusion) %>%
  summarize(r = cor(sd, n, use = "pairwise"))
```
...
rq1_plot_subs <- tibble(file = list.files("../../05-results/plots/RQ1/px-traj/png")) %>%
  separate(file, c("study", "SID", "emo_item", "er_exc"), sep = "-", remove = F) %>%
  filter(!is.na(er_exc)) %>%
  mutate(er_exc = str_remove(er_exc, ".png"))
save(rq1_plot_subs, file = "../../erdyn/rq1_plot_subs.RData")

rq2_int_plot_subs <- tibble(file = list.files("../../05-results/plots/RQ2-int/px-traj/png")) %>%
  separate(file, c("study", "SID", "emo_item", "ergoal", "er_exc"), sep = "-", remove = F) %>%
  filter(!is.na(er_exc)) %>%
  mutate(er_exc = str_remove(er_exc, ".png"))
save(rq2_int_plot_subs, file = "../../erdyn/rq2_int_plot_subs.RData")
```
...
comb_int_traj <- function(p, ergoal, foc, exc, er_exc){
  nr <- if(foc == "focal") 1 else 4
  nc <- 1
  wdth <- 12
  ht <- if(foc == "focal") 6 else 16
  ...
  p2 <- wrap_plots(p$p, ncol = nc, nrow = nr, byrow = F)
  ggsave(sprintf("../../05-results/plots/RQ2-int/%s-%s-%s-%s.png", ergoal, foc, exc, er_exc), width = wdth, height = ht)
  return(p2)
}
```
...
comb_p_rq2int <- p_rq2int %>%
  filter(!is.na(exclusion)) %>%
  mutate(focal = ifelse(emo_item %in% c("Positive", "Negative"), "focal", "sup")) %>%
  group_by(ergoal, exclusion, er_exc, focal) %>%
  nest() %>%
  ungroup() %>%
  mutate(p = pmap(list(data, ergoal, focal, exclusion, er_exc), comb_int_traj))
...
rq1_pred_plot <- function(study, sid, emo_item, file, exclusion, er_exc, colr){
  ...
  rqd <- "rq1"
  file2 <- str_remove_all(file, ".er|.all")
  if(er_exc == "er") rqd <- paste0(rqd, "-robustness")
  load(sprintf("../../02-data/02-clean-data/%s/rdata/%s", rqd, file2))
  data <- x
  ...
  filestr <- paste0(c(study, sid, emo_item, er_exc), collapse = "-")
...
nested_plots <- tibble(model = c("RQ1")) %>%
  ...
  full_join(emo_items %>% rename(emo_item = long_name)) %>%
  filter(exclusion == "parameter" & er_exc == "er") %>%
  # anti_join(
...
rq2_int_pred_plot <- function(study, sid, emo_item, ergoal, file, exclusion, er_exc){
  ...
  filestr <- paste0(c(study, sid, emo_item, ergoal, er_exc), collapse = "-")
...
nested_plots <- tibble(model = c("RQ2-int")) %>%
  ...
  # ) %>%
  filter(er_exc == "er" & exclusion == "parameter") %>%
  mutate(p = 
    future_pmap(
    # pmap(
...
```{r}
set.seed(12)
# cubic: -1(x-2.5)^3 + 2*(x-2.5)^2 1.5*(x-2.5) - 2.5

df2 <- tibble(Level = rnorm(116, 3, sd = 1)) %>%
  mutate(Y = -1*(Level-2.5)^3 + 2*(Level-2.5)^2 + 1.5*(Level-2.5) -2.5,
         Change = Y + rnorm(116, 0, 2)) %>%
  filter(abs(Change) < 4) %>%
...
p1 <- df2 %>%
  ggplot(aes(x = Time, y = Level)) +
    # geom_point(size = 1) +
    geom_line() +
    # geom_hline(aes(yintercept = mean(df2$Level)), linetype = "dashed", size = 1)  +
  labs(x = "Time (t)") + 
    theme_classic() +
    theme(legend.position = "none",
          strip.background = element_rect(fill = "black"),
          strip.text = element_text(face = "bold", color = "white", size = rel(1.2)),
          axis.text = element_blank(),#element_text(face = "bold", color = "black"),
...
p3 <- p2/p1
p3
ggsave(p3, file = "../../05-results/plots/time-series.png", width = 4, height = 4)
```

### Empirical Data

```{r}
ctsem::ctKalman(fit, plot = T)
test <- ctStanKalman(fit)
ctsem::plot.ctKalmanDF(fit)
...
plot_ts <- function(d, var){
  if(var == "dE_dt"){
    lims <- c(-10, 10)
    brk <- seq(-10, 10, 5)
    ylab <- expression("Emotion State Change " * frac(dE, dt))
  } else{
    lims <- c(0, 10)
    brk <- seq(0, 10, 2.5)
    ylab = expression("Emotion State (" * E[t*i] * ")")
  }
  d %>%
    ggplot(aes(x = time, y = y)) + 
      geom_line() + 
      geom_point() +
      scale_y_continuous(limits = lims, breaks = brk) + 
      labs(
        x = "Time (hours)"
        , y = ylab
        ) + 
...
rq1_ts_data_setup <- function(study, sid, emo_item, file, exclusion, er_exc){
  ei <- if(emo_item %in% c("Negative", "Positive")) paste(emo_item, "Emotion") else emo_item
  
  load(sprintf("../../05-results/RQ1/models/%s", file))
  fit
  
  coef <- as.matrix(summary(fit)$popmeans)[,"mean"] 
  
  rqd <- "rq1"
  file <- sprintf("%s-%s-%s.RData", study, sid, emo_item)
  if(er_exc == "er") rqd <- paste0(rqd, "-robustness")
  ...
  d <- data %>% 
    mutate(
      dE_dt = coef["mu_E"] + coef["beta_E"]*emo_value
    ) 
  
  filestr <- paste0(c(study, sid, emo_item, exclusion, er_exc), collapse = "-")
  save(d, file = sprintf("../../05-results/RQ1/predictions/%s.RData", filestr))
  return(d)
}

rq1_ts_plot <- function(study, sid, emo_item, file, exclusion, er_exc, colr){
  
  d <- rq1_ts_data_setup(study, sid, emo_item, file, exclusion, er_exc)
  
  d %>%
    select(SID, time, emo_value, dE_dt) %>%
    pivot_longer(
      cols = c(emo_value, dE_dt)
      , values_to = "y"
      , names_to = "var"
    ) %>%
    group_by(var) %>%
    nest() %>%
    ungroup() %>%
    mutate(p = map2(data, var, plot_ts))
  
  p <- wrap_plots(data_long$p, nrow = 2) +
    plot_annotation(
      title = sprintf("Time Series of %s and Change in %s", ei, ei)
      , subtitle = paste(str_to_upper(study), sid, sep = ": ")
      , theme = my_theme()
    )
  
  filestr <- paste0(c(study, sid, emo_item, exclusion, er_exc), collapse = "-")
  ggsave(p, file = sprintf("../../05-results/plots/time-series/%s.pdf", filestr), width = 8, height = 6)
  ggsave(p, file = sprintf("../../05-results/plots/time-series/png/%s.png", filestr), width = 8, height = 6)
  return(T)
}
```
...
ts_plots_nested <- tibble(model = c("RQ1")) %>%
  mutate(file = map(model, ~list.files(sprintf("../../05-results/%s/models", .)))) %>%
  unnest(file) %>%
  separate(file, c("study", "SID", "emo_item", "er_exc"), sep = "-", remove = F) %>%
  mutate_at(vars(er_exc), ~str_remove_all(., ".RData")) %>% 
  ...
  mutate(
    d = future_pmap(
      list(study, SID, emo_item, file, exclusion, er_exc)
      , rq1_ts_data_setup
      , .progress = T
    ))
 plan(sequential)
...
#### Who Returns to Their Attractor? 

```{r}
loadRData <- function(file, obj){
  file <- sprintf("../../05-results/RQ1/predictions/%s", file)
  load(file)
  get(ls()[grepl(obj, ls())])
}

nested_ts_pred <- tibble(file = list.files("../../05-results/RQ1/predictions/")) %>%
  mutate(data = map2(file, "d", loadRData))

ts_returns <- nested_ts_pred %>%
  separate(file, c("study", "SID", "emo_item", "exclusion", "er_exc"), sep = "-", remove = F) %>%
  mutate(er_exc = str_remove_all(er_exc, ".RData")) %>%
  filter(emo_item %in% c("Positive", "Negative")) %>% 
  select(-SID) %>%
  unnest(data) %>%
  group_by(study, emo_item, exclusion, er_exc, SID) %>%
  mutate(
    proximity = ifelse(abs(dE_dt) < .25, "near", "far")
    , lag_proximity = lag(proximity)
    , return = ifelse(is.na(lag_proximity), NA, ifelse(proximity == "near" & lag_proximity == "far", "return", "not_return"))
    ) 
  
ts_returns_sum <- ts_returns %>%
  filter(!is.na(return)) %>%
  group_by(study, emo_item, exclusion, er_exc, SID, return) %>%
  tally() %>%
  pivot_wider(
    names_from = "return"
    , values_from = "n"
    ) %>%
  mutate_at(vars(not_return, return), ~ifelse(is.na(.), 0, .)) %>%
  mutate(
    total = rowSums(cbind(not_return, return))
    , perc_return = return/total*100
    ) 

ts_returns_sum %>%
  group_by(study, emo_item, exclusion, er_exc) %>%
  summarize_at(
    vars(perc_return, return, total)
    , lst(median, sd, min, max)
    , na.rm = T
    )

ts_returns_gr <- ts_returns_sum  %>%
  mutate(returned = ifelse(return > 0, "yes", "no")) %>%
  group_by(emo_item, exclusion, er_exc, returned) %>%
  tally() %>%
  pivot_wider(
    names_from = "returned"
    , values_from = "n"
  ...
  mutate(
    total = rowSums(cbind(yes, no))
    , perc_return = yes/total*100
    ) %>%
  arrange(exclusion, er_exc, emo_item) 

tab_returns <- ts_returns_gr %>%
  kable(
    ., "html"
    , escape = F
    , align = rep("c", 7)
    , col.names = paste(paste("<strong>", c("Emotion Item", "Exclusion", "ER Only<br>Exclusion", "# no Return", "# Return", "# Participants", "% Return")), "</strong>")
    , digits = 2
    , caption = "<strong>Table S2</strong><br><em>Count and Percentage of Participants Who Demonstrated Returns to Model Estimated Attractors Based on Model Predicted Trajectories</em>"
    ) %>%
  kable_classic(full_width = F, html_font = "Times")
tab_returns
save_kable(tab_returns, file = "../../05-results/tables/tab-s2-returns.html")
```
...
tibble(model = c("RQ1")) %>%
  ...
  mutate(
    p = future_pmap(# pmap(
      list(study, SID, emo_item, file, exclusion, er_exc, color)
      , rq1_ts_plot # , possibly(rq1_ts_plot, NA_real_)
      , .progress = T
    ))
 plan(sequential)
 
 
ts_subs <- tibble(file = list.files("../../05-results/plots/time-series/png/")) %>%
  separate(file, c("study", "SID", "emo_item", "exclusion", "er_exc"), sep = "-", remove = F) %>%
  mutate(er_exc = str_remove_all(er_exc, ".png")) 
save(ts_subs, file = "../../erdyn/ts_subs.RData")
```
