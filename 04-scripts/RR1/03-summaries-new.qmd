...
nested_sum <- tibble(model = c("RQ1", "RQ2-int", "RQ2-main")) %>%
  ...
  separate(file, c("study", "SID", "emo_item", "ergoal", "er_exc"), sep = "-") %>% # look at this
...
```{r}
px_tab_fun <- function(x, model, sample, sid, emo_item, ergoal, er_exc){
  if(is.na(ergoal)) ergoal <- NULL
  exer <- paste(c("Positive", NULL), collapse=" x ")
  cap <- sprintf("<strong>TableSX</strong><br><em>All %s Parameter Estimates for %s for Participant %s", model, exer, sid)
  nt <- if(er_exc == "all") "All available data." else "Emotion regulation events only."
  filestr <- paste0(c(sample, sid, emo_item, ergoal, er_exc), collapse = "-")
  filestr <- paste0(
      "../../05-results/tables/"
      , model
      , "/px-tabs/"
  ...
  tab <- x %>%
    ...
    footnote(nt)
  save_kable(tab, file = filestr)
}

plan(multisession(workers = 12L))
nested_tabs <- summ_df %>%
  # filter(exclusion == "person") %>%
  group_by(model, study, SID, emo_item, ergoal, er_exc, exclusion) %>%
  nest() %>%
  ungroup() %>%
  mutate(tab = future_pmap(list(data, model, study, SID, emo_item, ergoal, er_exc), px_tab_fun, .progress = T))
plan(sequential)
closeAllConnections()
```
...
plan(multisession(workers=12L))
ma_nested <- summ_df %>%
  mutate(mean = ifelse(grepl("beta_E|strength", parameter), -1*mean, mean)) %>%
...
cor_tab_html_fun <- function(tab, focal, rq, exc, er_exc){
  ...
  save_kable(tab, file = sprintf("../../05-results/tables/RQ3/%s-correlations/%s-%s-%s.html", rq, focal, exc, er_exc))
  return(tab)
}
...
cor_tab_html_fun <- function(tab, focal, rq, exclusion, er_exc){
  ...
  save_kable(tab, file = sprintf("../../05-results/tables/RQ3/%s-correlations/%s-%s-%s.html", rq, focal, exclusion, er_exc))
  return(tab)
}
...
```{r}
col_ord <- emo_items %>% filter(!long_name %in% c("Positive", "Negative")) %>% pull(long_name)
col_ord <- paste(
  rep(col_ord, each = 6)
  , rep(c("goalNeg", "goalPos"), each = 24)
  , rep(c("location", "strength"), each = 3, times = 8)
  , rep(c("decrease", "maintain", "increase"), times = 16)
  , sep = "_"
)

nested_rq2int_sup_r <- summ_df %>% 
  filter(model == "RQ2-int" & grepl("location_|strength_", parameter)) %>%
  select(SID, exclusion, er_exc, emo_item, ergoal, parameter, mean) %>%
  filter((ergoal == "goalNeg" & emo_item != "Negative") | 
         (ergoal == "goalPos" & emo_item != "Positive")) %>%
  mutate(
    focal = ifelse(emo_item %in% c("Positive", "Negative"), "focal", "sup")
    ) %>%
  left_join(emo_items %>% select(category, emo_item = long_name)) %>%
  arrange(focal, emo_item, ergoal, parameter) %>%
  group_by(focal, category, exclusion, er_exc) %>%
  nest() %>%
  mutate(
    data = map(data, ~(.) %>% 
      pivot_wider(
...
mr_nested <- crossing(
  ...
  crossing(
    ...
    , wb_long %>%
      ...
      mutate(cat = ifelse(o_item %in% c("A", "C", "E", "N", "O"), "BFI2", "wb")) %>%
      full_join(
        erstrat %>% 
...
mr_fx <- mr_nested %>% 
  ...
  filter(
    ...
    (type == "quadratic" & term == "o_value_2" & o_item %in% c("DEP", "SWL"))
    ) %>%
  mutate(
    sig = ifelse(sign(conf.low) == sign(conf.high), "sig", "ns")
    , sig_sym = ifelse(p.value < .001, "‡", ifelse(p.value < .01, "†", ifelse(p.value < .05, "*", "")))
    , focal = ifelse(emo_item %in% c("Positive", "Negative") & o_item %in% c("SWL", "DEP"), "focal", "sup")
  ) %>%
  mutate_at(vars(estimate, conf.low, conf.high), ~sprintf("%.2f", .)) %>%
  mutate(
    CI = sprintf("[%s, %s]", conf.low, conf.high)
    , est = sprintf("%s%s<br>[%s, %s]", estimate, sig_sym, conf.low, conf.high)
    , estimate = sprintf("%s%s", estimate, sig_sym)
  ) %>%
...
rq3_rq1_tab_fun <- function(d, exclusion, focal, er_exc){
  ...
  tab <- d %>%
    ...
    add_header_above(h2) %>%
    add_header_above(h3)
  save_kable(tab, file = sprintf("../../05-results/tables/RQ3/tab-x-rq3b-emo-wb-correlates-%s-%s-%s.html", focal, exclusion, er_exc))  
  return(tab)
}
...
rq3_rq2_tab_fun <- function(d, exclusion, focal, er_exc){
  ...
  save_kable(tab, file = sprintf("../../05-results/tables/RQ3/tab-x-rq3b-int-wb-correlates-%s-%s-%s.html", focal, exclusion, er_exc))
  return(tab)
}
...
rq3b_2_tab %>% filter(exclusion == "parameter" & focal == "focal" & er_exc == "all") %>% pull(tab)
```
