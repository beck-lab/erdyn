...
cor_items <- cb %>% 
  ...
  distinct() %>%
  mutate(group = ifelse(grepl("BFI", long_name), "BFI-2", "Well-Being")) %>%
  full_join(
    cb %>% 
...
```{r}
h_tab <- read_csv("../../05-results/tables/tab-1-hypotheses.csv") %>%
  kable(
    .
    , "html"
    , caption = "<strong>Table 1</strong><br><em>Summary of Hypotheses Tested</em>"
    , align = c(rep("l", 3), rep("c", 5))
  ) %>%
  kable_classic(full_width = F, html_font = "Times") %>%
  column_spec(5:9, background = c(rep("white", 4), rep("grey80", 3), "white", rep("grey80", 7), "white")) %>%
  collapse_rows(1:2, "top") %>%
  footnote("+ = hypothesized positive association, - = hypothesized negative association, / = no directional hypothesis")
h_tab
save_kable(h_tab, file = "../../05-results/tables/tab-1-hypotheses.html")
```
