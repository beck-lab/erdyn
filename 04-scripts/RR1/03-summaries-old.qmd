...
# summ_df %>% 
#   filter(model == "RQ2-int" & any(flag == "implausible")) %>%
#   filter(emo_item %in% c("Positive", "Negative")) %>% View
# 
# summ_df %>%
#   filter(parameter == "beta_ExER") %>%
#   mutate(sig = ifelse(sign(lower) == sign(upper), "sig", "ns")) %>%
#   select(model:ergoal, sig) 
# 
# summ_df %>% 
#   filter(model == "RQ2-int" & flag == "implausible") %>%
#   arrange(desc(abs(mean)))

# summ_df %>%
#   group_by(SID, emo_item, ergoal, model) %>%
#   mutate(flag = ifelse(any(flag == "implausible"), "implausible", "plausible")) %>%
#   mutate(sig = ifelse(sign(lower) == sign(upper), "sig", "ns")) %>%
#   filter(parameter == "beta_ExER") %>%
#   group_by(emo_item, ergoal, flag, sig) %>% 
#   tally() %>%
...
# participants whose PA and NA models were excluded
pn_subs <- summ_df %>% filter(emo_item %in% c("Positive", "Negative")) %>% pull(SID) %>% unique()
subs <- summ_df %>% pull(SID) %>% unique() 
subs[!subs %in% pn_subs]

summ_df %>% 
  group_by(model, emo_item, ergoal, SID, exclusion) %>%
  slice_head(n = 1) %>%
  group_by(model, emo_item, ergoal, exclusion) %>%
  tally() %>% 
  pivot_wider(
    names_from = "emo_item"
    , values_from = "n"
...
summ_df %>% 
  filter(emo_item %in% c("Positive", "Negative") & model == "RQ1") %>%
  ggplot(aes(x = mean)) + 
  ...
  # filter(
  #   !(sd > 10 | 
  #   (parameter == "beta_E" & sign(mean) == 1) | 
  #   (grepl("beta_", parameter) & abs(mean) > 6) | 
  #   (parameter == "location" & abs(mean) > 10))
  #   ) 

summ_df %>%
  select(-sd, -lower, -mid, -upper) %>%
...
issues_nested <- nested_sum %>%
  ...
  mutate(flag = ifelse(
    (parameter == "beta_E" & sign(mean) == 1) | 
    (parameter == "location" & (mean > 10 | mean < 0))
    , "implausible", "plausible")) %>%
  filter(any(sd < 10) | any(flag == "implausible")) %>%
  ungroup()
```
...
```{r}
ma_fun <- function(d, mod, emo, er, param, exc){
  m <- rma(
    yi = mean
    , sei = sd
    , data = d
  ...
  #   , init = 0
  # )
  save(m, file = sprintf("../05-results/%s/ma-models/%s-%s-%s-%s.RData", mod, emo, er, param, exc))
  
  fx <- tidy(m, conf.int = T) %>%
    full_join(tibble(term = "sd", estimate = sqrt(m$tau2)))
  
  save(fx, file = sprintf("../05-results/%s/ma-fx/%s-%s-%s-%s.RData", mod, emo, er, param, exc))
  return(NULL)
}
...
ma_nested <- summ_df %>%
  ...
  mutate(ma = 
    future_pmap(
      list(data, model, emo_item, ergoal, parameter, exclusion)
...
ma_nested <- tibble(model = c("RQ1", "RQ2-int", "RQ2-main")) %>%
  ...
  separate(file, c("emo_item", "ergoal", "parameter", "exclusion"), sep = "-") %>%
    mutate(
      exclusion = str_remove_all(exclusion, ".RData")
      , ergoal = ifelse(ergoal == "NA", NA_real_, ergoal)
      ) %>%
  inner_join(ma_nested %>% select(model:exclusion)) %>%
  left_join(
    summ_df %>%
      mutate(mean = ifelse(grepl("beta_E|strength", parameter), abs(mean), mean)) %>%
      group_by(model, emo_item, ergoal, parameter, exclusion) %>% 
...
nested_ma <- ma_nested %>%
  ...
  mutate(
    ...
    , est = ifelse(term == "M", sprintf("%.2f<br>[%.2f,%.2f]", estimate, conf.low, conf.high), sprintf("%.2f", estimate))
    ) %>%
  select(-(estimate:sig)) %>%
  pivot_wider(
    names_from = "term"
  ...
  group_by(model, focal, exclusion) %>%
  nest() %>%
  ungroup()
```
...
```{r}
ma_tab_fun <- function(d, rq, focal, exc){
  if(rq == "RQ1"){
    d <- d %>% select(-ergoal)
  ...
  tab <- d %>%
    ...
    collapse_rows(1)
  save_kable(tab, file = sprintf("../05-results/tables/%s/key-terms/tabSx-%s-%s-%s-ma.html", rq, focal, str_to_lower(rq), exc))
  return(tab)
}
...
ma_tab_fun <- function(d, rq, focal, exclusion){
  ...
  tab
  save_kable(tab, file = sprintf("../05-results/tables/RQ2-int/key-terms/tabSx-rq2int-%s-%s-ma.html", focal, exclusion))
  return(tab)
}
...
mr_fun <- function(d, mod, emo, er, param, type, o_item, exc){
  ...
  #   , init = 0
  #   )
  save(m, file = sprintf("../05-results/%s/mr-models/%s-%s-%s-%s-%s-%s.RData", mod, emo, er, param, o_item, type, exc))
  
  fx <- tidy(m, conf.int = T)
  save(fx, file = sprintf("../05-results/%s/mr-fx/%s-%s-%s-%s-%s-%s.RData", mod, emo, er, param, o_item, type, exc))
  return(NULL)
}
...
rq3b_1_tab <- mr_fx %>%
  filter(model == "RQ1" & focal == "focal" & exclusion == "person") %>%
  select(-ergoal) %>%
  ...
  select(o_item, type, matches("Negative_beta"), matches("Negative_loc"), matches("Positive_beta"), matches("Positive_loc"), -exclusion) %>%
  kable(
    .
    , "html"
    , escape = F
  ...
  add_header_above(h3)
rq3b_1_tab
save_kable(rq3b_1_tab, file = "../05-results/tables/tab-x-rq3b-emo-wb-correlates-person.html")  
```
...
rq3b_2_tab <- mr_fx %>%
  ...
  select(o_item, direction, type, matches("Negative_str"), matches("Negative_loc"), matches("Positive_str"), matches("Positive_loc"), -exclusion) %>%
  mutate(
...
rq3b_2_tab
save_kable(rq3b_2_tab, file = "../05-results/tables/tab-x-rq3b-int-wb-correlates-person.html") 
```
...
nested_r <- summ_df %>% 
  ...
  select(model, SID, emo_item, ergoal, parameter, mean, exclusion) %>%
...
cor_tab_html_fun <- function(tab, focal, rq, exc){
  len <- nrow(tab)
  tab <- tab %>%
  ...
  save_kable(tab, file = sprintf("../05-results/tables/%s/correlations/%s-%s.html", rq, focal, exc))
  return(tab)
}
...
nested_rq1_r <- summ_df %>% 
  ...
  filter((parameter == "location" & mean > 0) | parameter == "beta_E") %>%
  select(SID, exclusion, emo_item, parameter, mean) %>%
  mutate(
    focal = ifelse(emo_item %in% c("Positive", "Negative"), "focal", "sup")
  ...
  mutate(
    ...
    , rtab_html = pmap(list(rtab, focal, "RQ1", exclusion), cor_tab_html_fun)
  )

nested_rq1_r %>% filter(focal == "focal") %>% pull(rtab_html)

rq1_r2 <- nested_rq1_r %>% 
  select(focal, r2, exclusion) %>%
  unnest(r2) %>%
  select(-term) %>%
  mutate_at(vars(estimate, conf.low, conf.high), ~sprintf("%.2f", .)) %>%
  mutate(
