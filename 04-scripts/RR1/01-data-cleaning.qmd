...
```{r}
icc_fun <- function(d){
  m0 <- lme4::lmer(value ~ 1 + (1 | SID), data = d)
  
  tau_00_2 <- VarCorr(m0)$SID[1]^2
  sigma_2 <- sigma(m0)^2
  icc <- tau_00_2/(tau_00_2 + sigma_2)
  return(icc)
}

ema_emo_icc <- emo_long %>% 
  full_join(ergoals_long) %>%
  select(
    SID, Date, item, value
    ) %>%
  group_by(item) %>%
  nest() %>%
  ungroup() %>%
  mutate(icc = map_dbl(data, icc_fun)) %>%
  select(-data)

tab_icc <- ema_emo_icc %>%
  kable(
    ., "html"
    , col.names = c("Item", "ICC")
    , align = c("l", "c")
    , digits = 2
    , caption = "<strong>Table SX</strong><br><em>Intraclass Correlation Coefficients of all EMA Variables</em>"
  ) %>%
  kable_classic(html_font = "Times", full_width = F)
tab_icc 
save_kable(tab_icc, file = "../../05-results/tables/tab-sx-iccs.html")
```
...
sc_ema_r_sum <- sc_ema_r %>%
  ...
  left_join(
    ema_desc %>% 
      left_join(
        ema_emo_icc %>%
          mutate(item = mapvalues(item, cb$item_name, cb$long_name, warn_missing = F))
        ) %>% 
      mutate(item = factor(item, levels = varL$vars, labels = varL$num)) %>%
      rename(variable = item)
    ) %>%
  select(variable, mean, omega_within, omega_between, icc, everything())

sc_ema_r_sum_tab <- sc_ema_r_sum %>%
    kable(
    .
    , "html"
    , digits = 2
    , escape = F
    , col.names = c("EMA Variable", "M (SD)", "&omega;<sub>w</sub><br>[CI]",
  "&omega;<sub>b</sub><br>[CI]", "ICC", paste0(1:12, "."))
    , align = c("l", rep("c", 17))
    , caption = "<strong>Table S1</strong><br><em>Average Within-Person Correlations and Descriptives for all EMA variables</em>"
  ) %>%
  kable_classic(html_font = "Times")
sc_ema_r_sum_tab
save_kable(sc_ema_r_sum_tab, file = "../../05-results/tables/tab-s1-within-rs-desc.html")

desc_subs <- tibble(file = list.files("../../05-results/tables/px-r-tabs")) %>% 
  separate(file, c("study", "sid", "scrap"), sep = "-", remove = F) 
save(desc_subs, file = "../../erdyn/desc-subs.RData")
```

## Baseline Data

```{r}
bl_df <- read_csv("../../02-data/01-raw-data/erdyn-baseline-2026-07-09.csv") %>%
...
bl_long <- bl_df %>%
  pivot_longer(
    cols = c(starts_with("CESD"), starts_with("SWL"), starts_with("BFI"))
    , names_to = "old_name"
    , values_to = "value"
  ) %>%
  left_join(
  ...
  mutate(maxi = ifelse(item == "CESD", 4, ifelse(item == "SWL", 7, 5)) 
...
bl_r_tab <- cor_tab_fun(bl_r) %>%
  ...
  kable( 
    ...
    , "html"
    , escape = F
    , col.names = c("Variable", "M (SD)", "&omega;<sub>w</sub><br>[CI]",
  "&omega;<sub>b</sub><br>[CI]", paste0(1:7, "."))
    , align = c("l", rep("c", 9))
    , caption = "<strong>Table 2</strong><br><em>Between-Person Descriptive Statistics and Correlations</em>"
  ) %>%
  kable_classic(html_font = "Times") %>%
  add_header_above(c(" " = 1, "Descriptives" = 3, "Correlations" = 7))
bl_r_tab
save_kable(bl_r_tab, file = "../../05-results/tables/tab-2-bp-desc.html")
```
